...
class KNeighborsVC(nn.Module):
    ...
    @torch.inference_mode()
    def get_features(self, path, weights=None, vad_trigger_level=0, isQuery=False):
        ...
        if torch.allclose(weights, self.weighting):
            ...
            features = features.squeeze(0) # (B, L, 1024)
        else:
            # use slower weighted
    ...
    def match(self, query_seq: Tensor, matching_set: Tensor, synth_set: Tensor = None, 
        ...
        query_seq = query_seq.to(device) # (B, seq_len, dim)
